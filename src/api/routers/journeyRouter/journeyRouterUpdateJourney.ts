...
import {
	IJourneyControllerUpdateJourneyOutputError as IControllerError,
	IJourneyControllerUpdateJourneyOutputSuccess as IControllerSuccess
} from '@app-ad/interfaces/api/controllers';

/**
 * @name					journeyRouterUpdateJourney
 *
 * @description		The 'updateJourney' router for 'journey', whose expected
 *                inputs have been defined within the following schema:
 *                'journeyValidationSchemaUpdateJourney'.
 *                
...
export const journeyRouterUpdateJourney = Router(options).patch(
  ...
  async (req: Request, res: Response) => {
    const errors = validationResult(req);
       ...
       * defined within 'journeyValidationSchemaUpdateJourney'.
       ...
       */
		  ...
		  const { journeyId, ...payload } = matchedData(req);
			const stat = await journeyController.updateJourney({ journeyId, payload });
